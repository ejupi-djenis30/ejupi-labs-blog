...
function assertSocialPreview(html, localeKey) {
  ...
  for (const key of alternateLocales) {
    assert.ok(
      html.includes(
  ...
  assert.equal(
    (html.match(/<meta property="og:locale:alternate" content=/gu) ?? []).length,
...
test("English case-study index links only to current canonical articles", async () => {
  const html = await readFile(new URL("../dist/index.html", import.meta.url), "utf8");
  for (const font of ["regular", "semibold"]) {
    assert.match(
      html,
      new RegExp(
        `<link rel="preload" href="/assets/fonts/instrument-sans-${font}\\.woff2" as="font" type="font/woff2" crossorigin />`,
        "u",
      ),
    );
  }
  ...
  assert.match(html, /data-case-slug="vector-placement-operations"/u);
  ...
  const blogSchema = JSON.parse(
    html.match(/<script type="application\/ld\+json">(?<json>.*?)<\/script>/su).groups.json,
  );
  assert.equal(blogSchema.datePublished, site.published);
  assert.equal(
    blogSchema.dateModified,
    currentCaseDefinitions.reduce(
...
test("technology tags are labelled semantic lists on indexes and articles", async () => {
  for (const localeKey of localeOrder) {
    ...
    for (const definition of visibleDefinitions) {
      ...
      assert.match(
        articleHtml,
        /<nav class="case-toc" aria-labelledby="case-contents-title">[\s\S]*?<span class="toc-title" id="case-contents-title">/u,
      );
      assert.match(
        articleHtml,
        /<div class="scope-note" role="note" aria-labelledby="scope-note-title"><strong id="scope-note-title">/u,
      );
      assert.doesNotMatch(
        articleHtml,
        /<aside class="(?:case-toc|scope-note)"/u,
      );
    }
  }
});
...
test("localized Labs articles keep canonical and hreflang routes aligned", async () => {
  ...
  assert.deepEqual(articleSchema.mainEntityOfPage, {
    "@type": "WebPage",
    "@id": "https://blog.ejupilabs.com/it/case-studies/careeros-local/",
  });
  assert.equal(articleSchema.articleSection, locales.it.cases["careeros-local"].category);
});
...
test("every article renders the two locale-safe related cases in ranked order", async () => {
  for (const localeKey of localeOrder) {
    ...
    for (const definition of caseDefinitions) {
      ...
      const expected = relatedCaseDefinitions(definition, currentCaseDefinitions, {
        localeKey,
        limit: 2,
      }).map(({ slug }) => slug);
...
test("sitemap and feed expose current work", async () => {
  ...
  assert.match(feed, /\/case-studies\/vector-placement-operations\//);
});

test("machine-readable catalog derives only current routes from the authoritative definitions", async () => {
  ...
  assert.equal(catalog.cases.length, currentCaseDefinitions.length);
  assert.equal(
    catalog.cases.some(({ slug }) => slug === "vector-placement-operations"),
    true,
  );
...
test("every lazy locale index contains article decisions, trade-offs and results", async () => {
  for (const localeKey of localeOrder) {
    ...
    assert.equal(index.cases.length, currentCaseDefinitions.length);
    assert.equal(
      index.cases.some(({ slug: entrySlug }) => entrySlug === "vector-placement-operations"),
      true,
    );
