...
export const labsLocales = {
  it: {
    "careeros-local": localize("careeros-local", {
      ...
      facts: [
        ...
        ["Stato", "Candidato v1.9.0 verificato al commit 253cde2; tag e pubblicazione sono ancora da completare"],
      ],
      evidence: {
        title: "Registro delle evidenze",
        intro: "Il repository documenta questi controlli e limiti riproducibili:",
        items: [
          ["Backend", "La suite di preparazione della v1.9.0 supera 1.532 test backend con 4 skip previsti e una branch coverage dell’81,42%."],
          ["Frontend + shell", "Passano 396 test frontend in 70 file. L’albero delle feature già integrato ha superato anche tutti i 17 test Rust, Clippy bloccato e i gate della supply chain."],
          ...
          ["Accesso agenti", "L’app desktop rilascia autorizzazioni con ambiti e revoca per sette strumenti CLI e MCP in sola lettura. Mostra il bearer una volta sola, mentre SQLite ne conserva soltanto il digest."],
        ],
      },
      starting: {
      ...
      delivery: {
        ...
        paragraphs: [
          "I controlli di preparazione del candidato v1.9.0 hanno superato 1.532 test backend con 4 skip previsti e branch coverage dell’81,42%, oltre a 396 test frontend in 70 file. L’albero delle feature già integrato ha superato tutti i 17 test Rust. Le suite di migrazione e archivio coprono il nuovo schema delle bozze, il formato v6 e il ripristino dalla v1 alla v5.",
          "CI protetta, CodeQL, controlli dei container e ciclo del pacchetto Linux x64 sono verdi sull’albero revisionato. Mancano ancora la prova in sola lettura sugli altri cinque target nativi per il commit finale, la revisione degli artefatti, il tag verificato e la release GitHub immutabile.",
        ],
      },
      result: {
        title: "Cosa esiste oggi",
        paragraphs: [
          "Il candidato v1.9.0 presente su main è una utility desktop funzionante con configurazione dal CV, Career Vault, ricerca guidata, Job Library revisionata, una timeline per opportunità, studio per il CV, bozze dossier persistenti, archivio v6 e analisi locale obbligatoria.",
          "L’app desktop autenticata ora gestisce l’accesso della CLI e del server MCP in sola lettura. Codex, Claude Code e script shell possono consultare una vista volutamente ridotta di un solo account autorizzato, ma non possono modificare il vault, invocare prompt liberi o aprire un trasporto remoto.",
      ...
      scope:
        "Questo case study descrive lo snapshot sorgente del candidato v1.9.0 al commit 253cde2. Il tag, la prova sui sei target nativi e la release immutabile non sono ancora completi. Non afferma risultati occupazionali, accuratezza del modello su dati privati né supporto per ogni modello locale e ogni macchina.",
    }),
  ...
  de: {
    "careeros-local": localize("careeros-local", {
      ...
      readMinutes: "14",
      facts: [["Produkt", "Open-Source-Desktop-Utility"], ["Rolle", "Produkt, Architektur und Implementierung"], ["Vertrauensgrenze", "Standardmäßig das lokale Gerät"], ["Status", "v1.9.0-Kandidat am Commit 253cde2 geprüft; Tag und Veröffentlichung stehen noch aus"]],
      evidence: { title: "Evidenzprotokoll", intro: "Das aktuelle Repository dokumentiert diese reproduzierbaren Prüfungen und Grenzen:", items: [
        ["Backend", "Die Vorbereitungssuite für v1.9.0 besteht 1.532 Backend-Tests mit 4 erwarteten Übersprüngen und 81,42% Branch Coverage."],
        ["Frontend + Shell", "Alle 396 Frontend-Tests in 70 Dateien bestehen. Der bereits integrierte Feature-Stand bestand außerdem alle 17 Rust-Tests, Locked Clippy und seine Supply-Chain-Prüfungen."],
        ...
        ["Agentenzugriff", "Die Desktop-App erteilt begrenzte, widerrufbare Berechtigungen für sieben schreibgeschützte CLI- und MCP-Werkzeuge. Sie zeigt das Bearer-Token einmal, während SQLite nur dessen Digest speichert."],
      ]},
      ...
      delivery: { title: "Wie das Produkt verifiziert wird", paragraphs: [
        "Die Vorbereitungstests des v1.9.0-Kandidaten bestanden 1.532 Backend-Tests mit 4 erwarteten Übersprüngen und 81,42% Branch Coverage sowie 396 Frontend-Tests in 70 Dateien. Der integrierte Feature-Stand bestand alle 17 Rust-Tests. Migrations- und Archiv-Suites decken das neue Entwurfsschema, Archiv v6 und die Wiederherstellung der Formate v1 bis v5 ab.",
        "Protected-Branch-CI, CodeQL, Containerprüfungen und der Lebenszyklus des Linux-x64-Pakets sind auf dem geprüften Feature-Stand grün. Vor der Veröffentlichung fehlen noch die schreibgeschützte Probe auf den anderen fünf nativen Zielen am exakten Release-Commit, die Artefaktprüfung, der verifizierte Tag und das unveränderliche GitHub-Release.",
      ]},
      result: { title: "Was heute vorhanden ist", paragraphs: [
        "Der v1.9.0-Kandidat auf main ist eine funktionsfähige Desktop-Utility mit Einrichtung aus dem Lebenslauf, Career Vault, geführter Suche, revisionsgeführter Job Library, einer Zeitleiste pro Opportunity, Lebenslauf-Studio, dauerhaften Dossierentwürfen, Archiv v6 und verpflichtender lokaler Analyse.",
        "Die authentifizierte Desktop-App verwaltet nun den Zugriff für die schreibgeschützte CLI und den MCP-Server über stdio. Codex, Claude Code und Shell-Skripte können eine bewusst kleine Ansicht eines autorisierten Kontos lesen, aber weder den Tresor ändern noch freie Prompts aufrufen oder einen entfernten Transport öffnen.",
      ...
      scope: "Diese Fallstudie beschreibt den Quell-Snapshot des v1.9.0-Kandidaten am Commit 253cde2. Tag, Probe auf sechs nativen Zielen und unveränderliches Release sind noch nicht abgeschlossen. Sie behauptet weder Beschäftigungsergebnisse noch Modellgenauigkeit auf privaten Benutzerdaten oder Unterstützung für jedes lokale Modell und jede Maschine.",
    }),
    "eliza-lab": localize("eliza-lab", {
      category: "Machine Learning",
  ...
  fr: {
    "careeros-local": localize("careeros-local", {
      ...
      readMinutes: "14",
      facts: [["Produit", "Utilitaire de bureau open source"], ["Rôle", "Produit, architecture et implémentation"], ["Périmètre de confiance", "Appareil local par défaut"], ["État", "Candidat v1.9.0 vérifié au commit 253cde2 ; tag et publication restent à terminer"]],
      evidence: { title: "Registre des preuves", intro: "Le dépôt actuel consigne ces vérifications et limites reproductibles :", items: [
        ["Backend", "La suite de préparation de la v1.9.0 réussit 1 532 tests backend avec 4 skips attendus et une couverture de branches de 81,42 %."],
        ["Frontend + shell", "Les 396 tests frontend répartis dans 70 fichiers réussissent. L’arbre des fonctionnalités déjà intégré a aussi réussi les 17 tests Rust, Clippy verrouillé et les contrôles de supply chain."],
        ...
        ["Accès des agents", "L’application de bureau délivre des autorisations limitées et révocables pour sept outils CLI et MCP en lecture seule. Elle montre le bearer une fois, tandis que SQLite ne conserve que son condensat."],
      ]},
      ...
      decisions: { title: "Les choix qui en font un véritable utilitaire", intro: "Le travail utile se déroule avant et après l’appel au modèle.", items: [
        ...
        { title: "Donner aux agents une porte séparée en lecture seule", body: "Agent Access demande à l’utilisateur connecté de choisir les périmètres et de s’authentifier à nouveau avant d’afficher une seule fois un bearer. Les autorisations expirent et restent révocables. La CLI et le serveur MCP exposent sur stdio un ensemble fermé d’outils en lecture seule, sans prompt libre ni API distante d’écriture.", tradeoff: "Les commandes installées depuis les sources demandent encore une configuration manuelle du client, et un client externe peut transmettre les données lues. Ce contrat étroit rend l’automatisation utile sans partager la session de bureau." },
      ]},
      delivery: { title: "Comment le produit est vérifié", paragraphs: [
        "Les contrôles de préparation du candidat v1.9.0 ont réussi 1 532 tests backend avec 4 skips attendus et 81,42 % de couverture de branches, ainsi que 396 tests frontend dans 70 fichiers. L’arbre des fonctionnalités intégré a réussi les 17 tests Rust. Les suites de migration et d’archive couvrent le nouveau schéma des brouillons, le format v6 et la restauration des formats v1 à v5.",
        "La CI de la branche protégée, CodeQL, les contrôles des conteneurs et le cycle du paquet Linux x64 sont au vert sur l’arbre relu. Il reste à exécuter la répétition en lecture seule sur les cinq autres cibles natives au commit exact de publication, puis à vérifier les artefacts, créer le tag vérifié et publier la version GitHub immuable.",
      ]},
      result: { title: "Ce qui existe aujourd’hui", paragraphs: [
        "Le candidat v1.9.0 présent sur main est un utilitaire de bureau fonctionnel avec démarrage depuis un CV, Career Vault, recherche guidée, Job Library révisionnée, une timeline par opportunité, studio de CV, brouillons de dossier persistants, archive v6 et analyse locale obligatoire.",
        "L’application de bureau authentifiée gère maintenant l’accès de la CLI et du serveur MCP en lecture seule. Codex, Claude Code et les scripts shell peuvent consulter une vue volontairement réduite d’un compte autorisé, mais ne peuvent ni modifier le coffre, ni invoquer de prompts libres, ni ouvrir de transport distant.",
      ...
      scope: "Cette étude de cas décrit le snapshot source du candidat v1.9.0 au commit 253cde2. Le tag, la répétition sur six cibles natives et la version immuable ne sont pas terminés. Elle ne revendique ni résultats professionnels, ni précision du modèle sur des données privées, ni prise en charge de tous les modèles locaux et de toutes les machines.",
    }),
    "eliza-lab": localize("eliza-lab", {
      category: "Machine learning",
